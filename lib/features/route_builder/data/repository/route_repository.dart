...
class RouteRepository {
  ...
  Future<List<RouteModel>> getRoutes() async {
    try {
      final response = await _supabase
          .from('routes')
          .select('''*, creator:creator_id(*), route_points:route_points(*)''')
  ...
  // Future<List<RouteWithPoints>> getFullRoute(String routeId) async {
  //   try {
  //     final response = await _supabase
  //         .from('routes')
  //         .select(
  //           '''*, creator:creator_id(*), route_points:route_points(*), way_points:way_points(*)''',
  //         )
  //         .eq('id', routeId)
  //         .order('created_at', ascending: false);
  //     final routes = response
  //         .map((response) => RouteWithPoints.fromJson(response))
  //         .toList();

  //     return routes;
  //   } catch (e) {
  //     throw AppException('Ошибка получения full маршрутов: $e');
  //   }
  // }
